...
namespace CalendarGridView
{
    ...
    class ItemCacheManager<T>
    {
        ...
        private Func<ItemIndexRange, CancellationToken, Task<T[]>> fetchDataCallback;
...
#endif
        public ItemCacheManager(Func<ItemIndexRange, CancellationToken, Task<T[]>> callback, int batchsize = 50, string debugName = "ItemCacheManager")
        {
        ...
        public void UpdateRanges(ItemIndexRange[] ranges)
        {
            ...
            for (int i = 0; i < ranges.Length; i++)
            {
                ...
                while (j < cacheBlocks.Count && cacheBlocks[j].FirstIndex <= ranges[i].LastIndex)
                {
                    ItemIndexRange overlap, oldEntryRange;
                    ItemIndexRange[] added, removed;
                    CacheEntryBlock<T> oldBlock = cacheBlocks[j];
            ...
            requests = new ItemIndexRangeList(ranges);
            ...
            foreach (ItemIndexRange range in ranges)
            {
                foreach (ItemIndexRange cached in cachedResults)
                {
        ...
        public ItemIndexRange GetFirstRequestBlock(int maxsize = 50)
        {
            if (requests.Count > 0)
            {
                ItemIndexRange range = requests[0];
        ...
        public void startFetchData()
        {
            ...
            {
                if (requests.Intersects(requestInProgress))
                {
                    return;
                }
                else
